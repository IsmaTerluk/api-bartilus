    ...
    class Login extends ConnectionDB{
        ...
        public function postLogin($json){
            ...
            if(isset($datos->{'email'}) && isset($datos->{'password'})){
                ...
                $encript_password = parent::encriptar($password);
                //Verifica existencia de usuario
                $exist = $this->existUser($email, $encript_password);
                if(isset($exist['rol'])){
        ...
        private function existUser($email, $password){
            $query = "SELECT rol FROM usuario NATURAL JOIN rol WHERE email='$email' AND password='$password'";
